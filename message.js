...
module.exports = {
	...
	execute(message, guild) {
		...
		//command stuff
		const channel = guild.channels.cache.find(channel => channel.name === 'log-tester') //finds the channel you want to log to

		if(message.author.tag === 'Client#6644') return; //Prevents the bot from logging itself creating an infinite log loop
		if(message.channel.type === 'dm') {
			channel.send(`[${data}] [${time}] [DMS] [${message.author.username}] : ${message.content}`)
			return;
		}
